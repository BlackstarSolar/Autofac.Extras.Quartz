...
namespace Autofac.Extras.Quartz
{
    ...
    public delegate bool JobRegistrationFilter([CanBeNull] Type jobType);
    ...
    public class QuartzAutofacJobsModule : Autofac.Module
    {
...
#if NETCORE
                .Where(type => !type.GetTypeInfo().IsAbstract && typeof(IJob).IsAssignableFrom(type) && FilterJob(type))
#else
        ...
        bool FilterJob([NotNull] Type jobType)
        {
